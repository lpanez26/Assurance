...
contract ownerSettings is Ownable {
    ...
    uint256 public period_to_withdraw = 2 minutes; //2 minutes for testing purpose
...
contract Assurance is ownerSettings, usingOraclize, SafeMath {
    ...
    mapping (address => address[]) contracts_waiting_approval; //incoming contracts for patient waiting his approval
    ...
    // ==================================== LOGIC ====================================
    //called by dentist
    function registerContract(uint256 _value_usd, uint256 _value_dcn, uint256 _date_start_contract, address _patient_addr) public validPatientDentistAddresses(_patient_addr, msg.sender) checkIfPaused {
        ...
        require(dentists[msg.sender].patients[_patient_addr].next_transfer == 0);
        //check if this dentist exists
        require(dentists[msg.sender].exists);

        //creating new subcontract between dentist and patient and it will be 'active' only once patient give his allowance to it
        address assurance_subcontract = new AssuranceSubcontract(msg.sender, _patient_addr, AssuranceContract);

        contracts_waiting_approval[_patient_addr].push(msg.sender);

        dentists[msg.sender].patients[_patient_addr] = patientStruct(_date_start_contract, false, _value_usd, _value_dcn, assurance_subcontract, dentists[msg.sender].patients_addresses.push(_patient_addr) - 1);
        emit logSuccessfulContractRegistration(msg.sender, _patient_addr, now, _value_usd, _value_dcn);
    }
    ...
    //called by AssuranceSubcontract
    function allowedWithdrawAmount(address _patient_addr, address _dentist_addr) public view checkIfPaused returns(bool _bool, uint256 _current_withdraw_amount) {
        //check if patient still allows the subcontract to use his DCN
        require(dcn.allowance(_patient_addr, msg.sender) > min_allowed_amount);

        //period_to_withdraw, api_result_dcn_usd_price, api_decimals
        if(!dentists[_dentist_addr].patients[_patient_addr].validation_checked) {
            //check if both patient and dentist address are valid
            require(_patient_addr != address(0) && _dentist_addr != address(0));
            //check if valid dentist and patient address are different
            require(_patient_addr != _dentist_addr);

            dentists[_dentist_addr].patients[_patient_addr].validation_checked = true;
        }

        //check if time passed for dentist to withdraw his dentacoins from patient
        require(now > dentists[_dentist_addr].patients[_patient_addr].next_transfer, "Please wait, you cannot withdraw Dentacoins yet.");

        uint256 months_num = 1;

        //time range from the last withdraw from this patient till now
        uint256 time_range = now - dentists[_dentist_addr].patients[_patient_addr].next_transfer;
        ...
        dentists[_dentist_addr].patients[_patient_addr].next_transfer = now + period_to_withdraw;
        ...
        if(usd_over_dcn) {
            ...
            current_withdraw_amount = div(mul(dentists[_dentist_addr].patients[_patient_addr].value_usd, 10**api_decimals), api_result_dcn_usd_price);
            ...
            //IF DCN MATTERS
            current_withdraw_amount = mul(dentists[_dentist_addr].patients[_patient_addr].value_dcn, months_num);
        }

        return (true, current_withdraw_amount);
    }
    ...
    function getPatient(address _patient_addr, address _dentist_addr) public view validPatientDentistAddresses(_patient_addr, _dentist_addr) returns(address, address, uint256, uint256, uint256, address) {
        patientStruct memory patient = dentists[_dentist_addr].patients[_patient_addr];
        return (_dentist_addr, _patient_addr, patient.next_transfer, patient.value_usd, patient.value_dcn, patient.sub_contract_addr);
    }
    ...
    function getWaitingContractsForPatient(address _patient_addr) public view returns(address[]) {
        return contracts_waiting_approval[_patient_addr];
    }
    // ====== /GETTERS ======
...
contract AssuranceSubcontract {
    // ==================================== STATE ====================================
    address public patient;
    address public dentist;
    Assurance assurance_contract; // THIS WILL BE HARDCODED WHEN UPLOADING TO TESTNET/MAINNET
    DentacoinToken dcn = DentacoinToken(0x19f49a24c7cb0ca1cbf38436a86656c2f30ab362); // THIS WILL BE HARDCODED WHEN UPLOADING TO TESTNET/MAINNET

    constructor(address _dentist_addr,address _patient_addr, address _assurance_contract) {
        dentist = _dentist_addr;
        patient = _patient_addr;
        assurance_contract = Assurance(_assurance_contract); // THIS WILL BE GONE WHEN UPLOADING TO TESTNET/MAINNET
    }
    // ==================================== /STATE ====================================
    ...
    function withdraw() public {
        ...
        //response from Assurance contract method allowedWithdrawAmount
        (allowance, amount) = assurance_contract.allowedWithdrawAmount(patient, dentist);

        //here we check if the dentist is ready to continue with the withdraw
        require(allowance == true);

        dcn.transferFrom(patient, msg.sender, amount);
        emit logSuccessfulWithdraw(msg.sender, patient, amount, now);
    }
    // ==================================== /LOGIC ====================================
}