...
contract Assurance is ownerSettings, usingOraclize, SafeMath {
    ...
    mapping (address => patientContractHistory) contracts_waiting_approval; //incoming contracts for patient, waiting his approval
    ...
    function registerContract(address _patient_addr, address _dentist_addr, uint256 _value_usd, uint256 _value_dcn, uint256 _date_start_contract) public validPatientDentistAddresses(_patient_addr, _dentist_addr) checkIfPaused {
        ...
        require(dentists[_dentist_addr].patients[_patient_addr].next_transfer == 0, "Only one contract is allowed per pair of patient and dentist.");


        //if dentist is the one who calls the method the patient should approve the new contract
        if(msg.sender == _dentist_addr) {
            dentists[_dentist_addr].patients[_patient_addr] = patientStruct(_date_start_contract, true, false, false, _value_usd, _value_dcn, dentists[_dentist_addr].patients_addresses.push(_patient_addr) - 1);

            //inserting dentistSentProposal so patient can track his incoming contracts from dentist
            contracts_waiting_approval[_patient_addr].dentists[_dentist_addr] = dentistSentProposal(contracts_waiting_approval[_patient_addr].dentists_addresses.push(_dentist_addr) - 1, true);
        }else {
            dentists[_dentist_addr].patients[_patient_addr] = patientStruct(_date_start_contract, false, true, false, _value_usd, _value_dcn, dentists[_dentist_addr].patients_addresses.push(_patient_addr) - 1);
        }
    ...
    function dentistApproveContract(address _patient_addr) public validPatientDentistAddresses(_patient_addr, msg.sender) checkIfPaused {
        dentists[msg.sender].patients[_patient_addr].approved_by_dentist = true;
    ...
    function patientApproveContract(address _dentist_addr) public validPatientDentistAddresses(msg.sender, _dentist_addr) checkIfPaused {
        dentists[_dentist_addr].patients[msg.sender].approved_by_patient = true;
    ...
    function withdrawToDentist(address _patient_addr) public checkIfPaused {
        ...
        if(!dentists[msg.sender].patients[_patient_addr].validation_checked) {
            ...
            require(dentists[msg.sender].patients[_patient_addr].approved_by_dentist && dentists[msg.sender].patients[_patient_addr].approved_by_patient, "In order contract to be active both patient and dentist must agree with the contract terms.");
    ...
    function breakContract(address _patient_addr, address _dentist_addr) public validPatientDentistAddresses(_patient_addr, _dentist_addr) checkIfPaused {
        ...
        require(dentists[_dentist_addr].patients[_patient_addr].next_transfer > 0, "This patient is not in contract with this dentist.");

        //if there is proposal recorded from this dentist for this patient ----> delete it
        if(contracts_waiting_approval[_patient_addr].dentists[_dentist_addr].exists) {
            //deleting the dentist address from the dentists_addresses array for the current patient
            uint256 proposal_row_to_delete = contracts_waiting_approval[_patient_addr].dentists[_dentist_addr].index_in_dentists_addresses;
            address proposal_key_to_move = contracts_waiting_approval[_patient_addr].dentists_addresses[contracts_waiting_approval[_patient_addr].dentists_addresses.length-1];
            contracts_waiting_approval[_patient_addr].dentists_addresses[proposal_row_to_delete] = proposal_key_to_move;
            contracts_waiting_approval[_patient_addr].dentists[proposal_key_to_move].index_in_dentists_addresses = proposal_row_to_delete;
            contracts_waiting_approval[_patient_addr].dentists_addresses.length--;

            //deleting the struct
            delete contracts_waiting_approval[_patient_addr].dentists[_dentist_addr];
        }
    ...
    function getPatient(address _patient_addr, address _dentist_addr) public view validPatientDentistAddresses(_patient_addr, _dentist_addr) returns(address, address, uint256, bool, bool, bool, uint256, uint256) {
        patientStruct memory patient = dentists[_dentist_addr].patients[_patient_addr];
        return (_dentist_addr, _patient_addr, patient.next_transfer, patient.approved_by_dentist, patient.approved_by_patient, patient.validation_checked, patient.value_usd, patient.value_dcn);
    }
    ...
    function getWaitingContractsForPatient(address _patient_addr) public view returns(address[]) {
        return contracts_waiting_approval[_patient_addr].dentists_addresses;
    }
    // ====== /GETTERS ======
