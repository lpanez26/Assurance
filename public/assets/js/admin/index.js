...
//saving image alts on media listing pages
function saveCalculatorParameters()   {
    if($('.save-calculator-parameters').length > 0)    {
        $('.save-calculator-parameters').click(function()  {
            var object = {};
            var tbody_trs = $('.calculator-parameters-container table tbody tr');
            for(var i = 0, len = tbody_trs.length; i < len; i+=1)  {
                object[tbody_trs.eq(i).attr('data-id')] = {
                    param_gd_cd_id: tbody_trs.eq(i).find('#param_gd_cd_id').val().trim(),
                    param_gd_cd: tbody_trs.eq(i).find('#param_gd_cd').val().trim(),
                    param_gd_id: tbody_trs.eq(i).find('#param_gd_id').val().trim(),
                    param_cd_id: tbody_trs.eq(i).find('#param_cd_id').val().trim(),
                    param_gd: tbody_trs.eq(i).find('#param_gd').val().trim(),
                    param_cd: tbody_trs.eq(i).find('#param_cd').val().trim(),
                    param_id: tbody_trs.eq(i).find('#param_id').val().trim(),
                };
            }
            $.ajax({
                type: 'POST',
